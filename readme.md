...
### 特定座標(中央座標)移動関数, moveToPosition moveToCenter関数の仕様

取得した座標を用いて，その座標に移動する関数です．この機能を使用するには以下の設定を行ってください．
- 言語設定を英語にする．(マイクラトップの言語設定から変更可能)
- ステージの設定から，座標の表示を有効化する．(game -> show cordinates)

この関数には以下の欠点があります．
- 移動中，ゾンビの攻撃のノックバックによりプレイヤの座標が大きく変わると正常に動作　しなくなる可能性があります．
- 指定した座標には進まず，その周辺５ブロック以内に移動する可能性が非常に高いです．
